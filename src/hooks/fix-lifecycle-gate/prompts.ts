...
export function buildCaptureBlockMessage(verdict: CaptureVerdict): string {
	return [
		...
		"",
		"必须立即完成以下操作：",
		"1. **经验教训 (L-xxx)** — 更新 _meta/knowledge/lessons-learned.md（问题→诊断→修复→提炼）",
		"2. **已知陷阱 (P-xxx)** — 更新 _meta/knowledge/pitfalls.md（症状→根因→修复）（如有）",
		"3. **架构决策 (D-xxx)** — 更新 _meta/knowledge/decisions.md（背景→方案→理由）（如有）",
		"4. **约束登记 (C-xxx)** — 更新 _meta/knowledge/constraints.md（约束→级别→来源）（如有）",
		"5. 如确认无新增知识，显式声明「Capture: 无新增知识沉淀」",
		"",
